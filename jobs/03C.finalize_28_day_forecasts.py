# 03C.finalize_28_day_forecasts.py
...
def main():
    ...
    # Load Parameters
    job_timestamp_utc = dbutils.jobs.taskValues.get(taskKey="Preprocess28DayForecasts", key="job_run_date")
    ...
    prediction_table_name = "intermediate_28_day_predictions"
    test_set_table_name = "feature_set"
    full_table_name = f"{catalog_name}.{schema_name}.Dept_Store_28_Day_Forecast"
    ...
    predictions_df = spark.table(f"{catalog_name}.{schema_name}.{prediction_table_name}")
    ...
    final_predictions_df = final_predictions_df.withColumn("update_predictions_timestamp_utc", F.lit(job_timestamp_utc_timestamp))
    ...
    if not table_exists(full_table_name):
        ...
        # Create the table
        final_predictions_df.write.format("delta").mode("overwrite").saveAsTable(full_table_name)
    else:
        print(f"Table {full_table_name} exists. Proceeding with MERGE operation.")
    ...
    spark.sql(f"DROP TABLE IF EXISTS {catalog_name}.{schema_name}.{prediction_table_name}")
